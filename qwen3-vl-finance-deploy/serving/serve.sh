...
# FP8 KV Cache 量化: 显存减半, 同等显存下 batch size 翻倍
# 需要 GPU CC >= 8.0 (Ampere+), CC >= 8.9 (Ada/Hopper) 有原生 FP8 支持
export ENABLE_FP8_KV_CACHE="${ENABLE_FP8_KV_CACHE:-0}"
...
echo "  FP8 KV Cache:    $ENABLE_FP8_KV_CACHE"
echo "============================================"
...
if [ "$DEPLOY_MODE" = "disagg" ]; then
    ...
    # 单实例模式: 同进程内 patch + 启动, 确保 CUDA Graph 捕获自研 kernel
    echo ""
    echo "[单实例模式] 同进程 patch → 启动 vLLM OpenAI Server"
    echo ""

    SERVE_ARGS="--model $BASE_MODEL \
        --host $HOST --port $PORT \
        --max-model-len $MAX_MODEL_LEN \
        --gpu-memory-utilization $GPU_UTIL \
        --trust-remote-code \
        --quantization compressed-tensors \
        --enable-lora --max-loras 2 --max-lora-rank $LORA_RANK --max-cpu-loras 2 \
        --lora-modules finance-expert-a=$EXPERT_A finance-expert-b=$EXPERT_B \
        --limit-mm-per-prompt {\"image\":4,\"video\":1}"

    if [ "$ENABLE_CHUNKED_PREFILL" = true ]; then
        SERVE_ARGS="$SERVE_ARGS --enable-chunked-prefill --max-num-batched-tokens $MAX_NUM_BATCHED_TOKENS"
    fi
    if [ "$ENABLE_PREFIX_CACHING" = true ]; then
        SERVE_ARGS="$SERVE_ARGS --enable-prefix-caching"
    fi

    CUDA_VISIBLE_DEVICES=0 python3 serve_launcher.py $SERVE_ARGS
fi
