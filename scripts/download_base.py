#!/usr/bin/env python3
"""
从 ModelScope 下载 Qwen2.5-32B-Instruct 基座模型，供 vLLM 使用。
...
# ModelScope 上的 Qwen2.5-32B-Instruct 基座（与 HuggingFace 同源）
DEFAULT_BASE_MODEL_ID = "Qwen/Qwen2.5-32B-Instruct"
...
def download_base(
    ...
    print(f"正在从 ModelScope 下载基座模型: {model_id}")
    print("  （约 65GB，请确保磁盘空间充足，下载时间视网络而定）")
...
def main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="从 ModelScope 下载 Qwen2.5-32B-Instruct 基座模型")
    parser.add_argument(
        "--model-id",
        type=str,
    ...
    path = download_base(model_id=args.model_id, cache_dir=args.cache_dir)
    print(f"\n下载完成。启动服务时使用：")
